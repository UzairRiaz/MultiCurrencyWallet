...
export const prettyAmount = (amount: Amount) => new BigNumber(amount.amount).shiftedBy(-amount.decimals).toFixed()
...
export const executeSolanaTransaction = async (
  ...
  if (tx.serializedMessage != null) {
    ...
    tx.signatures.forEach((signatureItem, index, array) => {
  ...
  try {
    if (!transaction.recentBlockhash) { transaction.recentBlockhash = (
