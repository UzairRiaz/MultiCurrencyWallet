...
const uint8ArrayToHex = (buffer: Uint8Array): string => [...buffer]
...
export async function executeCosmosTransaction(cosmosTx: CosmosTransaction): Promise<string> {
  ...
  function manipulateMsg(m: any): any {
    ...
    if (m.__type === 'DirectCosmosIBCTransferMessage') {
      ...
      if (result.value.timeoutTimestamp) result.value.timeoutTimestamp = Long.fromString(result.value.timeoutTimestamp) as any
      if (!!result.value.timeoutHeight?.revisionHeight) result.value.timeoutHeight.revisionHeight = Long.fromString(result.value.timeoutHeight.revisionHeight) as any
      if (!!result.value.timeoutHeight?.revisionNumber) result.value.timeoutHeight.revisionNumber = Long.fromString(result.value.timeoutHeight.revisionNumber) as any
      return result
    }
    return { ...m }
  }
